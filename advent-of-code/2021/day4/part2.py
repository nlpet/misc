...
def mark_boards(numbers, mappings, scores, size):
    disqualified_boards = set([])
    num_boards = len(scores)

    for number in numbers:
        for n in range(num_boards):
            if n in disqualified_boards:
                continue
            ...
            if mappings[n].get(number):
                ...
                if scores[n]["rows"][row] == size or scores[n]["cols"][col] == size:
                    if len(disqualified_boards) < num_boards - 1:
                        disqualified_boards.add(n)
                        continue
...
def solution():
    ...
    with open("input.txt") as fr:
        ...
        # Mark boards and find last winner
