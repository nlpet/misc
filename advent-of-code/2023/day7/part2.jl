...
function hand_type(hand::AbstractString)
    hand_no_jokers = filter(i -> (i !== 'J'), hand)
    if length(hand_no_jokers) == 0
        hand_repr = [5]
    else
        hand_repr = collect(values(countmap(hand_no_jokers)))
        sort!(hand_repr, rev=true)
    end

    jokers = count(i -> (i == 'J'), hand)
    hand_repr[1] = min(hand_repr[1] + jokers, 5)
...
function compare_hands(hand1::AbstractString, hand2::AbstractString)
    ...
    for i in 1:min(length(values1), length(values2))
        if values1[i] != values2[i]
            values1[i] = values1[i] == 11 ? 1 : values1[i]
            values2[i] = values2[i] == 11 ? 1 : values2[i]
